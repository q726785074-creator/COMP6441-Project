...
The project aims to help university students with little or no formal cybersecurity education recognise common phishing attacks and respond more safely before and after an interaction.

## What we built

PhishWise is an interactive phishing education website that runs locally in a browser. It includes bilingual learning content, simulated email analysis, scenario-based response decisions, a ten-question pre-test, a different ten-question post-test, score comparison, review guidance, and anonymous local feedback.

## Expected outcomes

The following outcomes are projected from the activity design, teaching sequence, and assessment mechanism. They are anticipated learning outcomes, not findings from real participant testing.

- Learners are expected to identify suspicious senders, links, attachments, and urgency language more accurately.
- Learners are expected to rely less on branding, display names, and visual design when judging legitimacy.
- Learners are expected to verify requests through an independent trusted channel before clicking or entering credentials.
- Learners are expected to choose safer responses in simulated phishing situations.
- Post-test performance is expected to be higher than pre-test performance, but real participant evidence is still required to evaluate this projection.
...
Demo Mode is available in the top-right corner for a classroom presentation. It unlocks every section and can create values labelled Hypothetical demonstration data. These simulated results are for demonstration purposes only, remain separate from formal progress, and are cleared when the page is refreshed or Demo Mode is exited.
